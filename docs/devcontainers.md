...
honored in full — image or build, lifecycle hooks, mounts, `runArgs` —
...
whose config asks for a host mount gets it once a human has approved that config.

There are two exceptions, both where honoring the config would make the human
approval uninformative rather than permissive: `privileged` and the
capability/namespace grants equivalent to it
are refused, because they let the container reach host storage the approved config
never named. `features` are refused for the same reason from the other side — the
CLI fetches a Feature's metadata from a registry and merges it into the config at
build time, so that metadata can add `privileged`, capabilities or host mounts
that the approved text never contained and no screen here can see. See "What is
screened" below.
...
`--ipc=host`, `--uts=host`, `--userns=host`, `--cgroupns=host`, and each of their
Compose equivalents: `privileged: true`, `cap_add:`, `network_mode: host`, `pid:
host`, `ipc: host`, `uts: host`, `userns_mode: host`, `cgroup: host`).
