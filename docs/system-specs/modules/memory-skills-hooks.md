...
**sync_discovered_servers()**: the one serialized discover→write entry point (`discover` + agent-config rebuild + Claude Code sidecar) shared by `POST /api/mcp/sync`, the sessions-restart pre-sync, and the config watcher. A module mutex serializes concurrent callers, closing the read-modify-write race the two handlers used to have.
...
**Dashboard endpoints**: GET `/api/mcp` (list with enabled state from installed config), GET `/api/mcp/probe` (cached probe results, non-blocking), POST `/api/mcp/probe` (live probe all, updates cache), GET `/api/mcp/config-status` (config generation + `sessionsStale`, the bit behind the "restart sessions to apply" banner), POST `/api/mcp/sync` (on-demand discover + add + session reset), POST `/api/mcp/toggle` (enable/disable in installed config).
