...
//
// Backed by a module-level store (not usePersistedBool) because the top strip
// and a remote pane's embedded bar can be mounted at the SAME time — the strip
// is display:none but still mounted behind an active remote pane (see
// InstancesViewport). Both must flip the instant the pin is toggled; a
// per-instance hook would leave the hidden bar on its stale value until a
// remount, so the preference would look like it didn't take after a pane switch.
 ...
 * names the pane on screen instead.
 */
 ...
 *
 * The expanded row's overflow strategy is variant-specific: the full-width
 * `strip` overlay WRAPS so every crew is visible at once, while the fixed-height
 * `inline` header stays one line and scrolls, capped so the row never runs under
 * the centered top-bar search.
 */
function Switcher({
  entries,
  activeId,
    ...
    <div className="flex items-center gap-1 min-w-0">
      ...
      <div
        className={
          ...
          (variant === 'strip'
            ? 'flex-wrap'
            : 'flex-nowrap overflow-x-auto max-w-[42vw]')
        }
      >
        {entries.map(entry => (
          <SwitcherChip
