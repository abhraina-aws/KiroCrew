...
function flankingFor(
  ...
  const punctBefore = UNICODE_PUNCT_RE.test(before)
  const punctAfter = UNICODE_PUNCT_RE.test(after)
  const leftFlanking = !wsAfter && (!punctAfter || wsBefore || punctBefore)
  const rightFlanking = !wsBefore && (!punctBefore || wsAfter || punctAfter)
  // `_` additionally cannot do intraword emphasis; `*` can.
  if (ch === '_') {
    return {
...
function hasPendingStrongOpener(line: string, upTo: number, delim: string): boolean {
  ...
  while (i < upTo) {
    ...
    if (end > from) {
      ...
      if (canOpen) open += end - from
      else if (canClose) open -= Math.min(end - from, open)
    }
    i = end
  }
 ...
 */
function strongDelimCutIndex(run: string, prefix: string): number {
 ...
 * conservative choice.
 */
function earliestCut(run: string, prefix: string): number {
  const cuts = [cjkCutIndex(run, prefix), strongDelimCutIndex(run, prefix)].filter((i) => i > 0)
...
export function fixCjkAutolinkBoundaries(content: string): string {
  ...
  for (const [start, end] of literals) {
    ...
    while ((m = URL_START_RE.exec(content.slice(start, end))) !== null) {
      ...
      const cut = earliestCut(run, prosePrefix(content, nonProse, at))
      if (cut < 0) {
