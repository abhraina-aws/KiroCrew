...
def _caller_app_scope(caller_key: str, rows: list[dict]) -> str:
    """The app owning the calling session, or "" when it owns none.
    ...
    A caller with NO row is unscoped, because only a dashboard chat slot can be
    app-owned — a Slack thread, a cron or a channel session has no slot to carry
    an app, and refusing those would deny the list to callers that never had an
    app to be confined to. An app agent always has its slot (that is where its
    app tag lives), so this does not loosen the app case.
    """
