...
def _remove_ignorable_dir(path: Path) -> bool:
    ...
    directory content-free). The marker is unlinked first, then the directory
    is removed with ``os.rmdir`` — kernel-atomic: it succeeds only if the
    directory is STILL empty at unlink time, so a file created through a
    lingering directory handle after the emptiness check makes this return
    False instead of being lost. Callers must preserve the tree on False.
    """
    try:
        (path / _PROVENANCE_MARKER).unlink(missing_ok=True)
        os.rmdir(path)
    except OSError:
        return False
    return True
