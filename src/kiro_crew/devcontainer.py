...
def _assert_compose_service_unprivileged(svc: dict, service_name: str) -> None:
    ...
    # Compose's own names for the host-namespace modes.
...
#: Short spellings docker accepts for a long flag.
_RUN_FLAG_CANON = {"-v": "--volume"}
...
def assert_no_unscreened_features(cfg: dict) -> None:
    """Refuse ``features``: their metadata is remote, merged, and unscreenable.
...
def assert_no_privileged_modes(cfg: dict) -> None:
    ...
    for canon, value in _iter_run_flags(flags):
        ...
        if canon in _HOST_NAMESPACE_FLAGS and value and value.strip().lower() == "host":
            raise DevcontainerError(
                f"devcontainer runArgs set {canon}=host, which places the container "
                f"in the host namespace -- the isolation the trust grant is premised "
                f"on is not created at all"
            )
