...
from kiro_crew.env import emit_env, sanitize_spec_env, spec_env_path
...
async def probe_server(
    ...
    *client_info* overrides the ``clientInfo`` sent in the handshake. The
    ...
    try:
        ...
        # expanded.
        env["PATH"] = spec_env_path(server.env.get("PATH", ""))
        ...
        env.update(
            sanitize_spec_env((k, v) for k, v in server.env.items() if k != "PATH")
        )
