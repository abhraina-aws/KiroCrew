...
class TestSanitizeSpecEnv:
    ...
    def test_python_env_passes_through(self) -> None:
        """PYTHONPATH/PYTHONSTARTUP take effect inside the SERVER process —
        already sandboxed, already running spec-author code by definition —
        and kiro-cli sessions receive them untouched from the emitted config.
        Dropping them here would report an error for a server that works in
        every session: the mirror image of the divergence this closes."""
        pairs = [("PYTHONPATH", "/srv/lib"), ("PYTHONSTARTUP", "/srv/rc.py")]
