...
class TestAFailedRevertIsNotReportedAsSuccess:
    ...
    def test_recovery_reports_which_targets_it_could_not_put_back(
        self, tmp_path: Path
    ) -> None:
        ...
        failed = snap._restore_everything_from_rollback(
            backup, home, ["workspace"], {"workspace"}
        )
...
class TestTheOutboundScrubCoversWhatItClaims:
    ...
    def test_the_tag_cannot_be_matched_as_a_value(self) -> None:
        ...
        """
        assert " " in redact._TAG, (
            "the tag must contain whitespace, which the value class excludes"
        )
